/* ast.c
 * Adaptado por Christina e modificado pela equipe: Natan Moura e Pedro Augusto */
...
struct decl *const_decl_create(char *name, struct type *type, int i){
    struct expr *ci = expression_create(EXPR_INTEGER_LITERAL,0,0,0,i);
    struct decl *cd = decl_create(name, type, ci, 0, 0);
    return cd;
}
...
//Olhar 1:36:43
// era enum_decl_create
// Tipo Enum com uma lista
/*struct decl * enum_type_create(char *name, struct param_list *idlist)
{
    struct type *et = type_create(TYPE_ENUM,0, idlist);
    // int enum_size = 0;
    struct decl *ed = decl_create(name,et,0,0,0);
    return ed;
}

// tipo | ENUM ID ID ';'
struct decl * enum_var_decl_create(char *varname, char *typename)
{
    struct type *et = type_create(TYPE_ENUM,0, 0);
    struct expr *e = expr_create_name(typename);
    struct decl *ed = decl_create(varname,et,e,0,0);
    return ed;
}*/
...
struct stmt * for_create(
    struct expr *init_expr,
    struct expr *expr,
    struct expr *next_expr,
    struct stmt *body){
    struct stmt *fs =
        stmt_create(STMT_FOR,0,init_expr,expr,next_expr,body,0,0);
    return fs;
