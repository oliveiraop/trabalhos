...
struct param_list *insert_param(struct param_list *head, struct param_list *elem)
{
...
struct decl * decl_create(char *name, struct type *type, struct expr *expr, struct stmt *code, struct decl *next )
{
...
{
    struct expr *ei = expression_create(EXPR_INTEGER_LITERAL,0,0,0,i);
    return ei;
}
...
struct decl * const_decl_create(char *name, struct type *type, struct expr *expr)
{
    struct type *ct = type_create(TYPE_CONST, type, 0);
    struct decl *cd = decl_create(name, ct, expr, 0, 0);
    return cd;
}



struct  stmt * for_create(struct expr *expr1, struct expr *expr2, struct expr *expr3, struct stmt *stmt)
{
    struct stmt *fs = stmt_create(STMT_FOR, 0, expr1, expr2, expr3, stmt, 0, 0);
    return fs;
}
...
{
    struct type *at = type_create(TYPE_ARRAY,type,0);
    struct param_list *pl = param_create(name,at);
    return pl;
}
...
struct stmt * compound_stmt_create(stmt_t kind, struct decl *localdecl, struct stmt *body)
{
    struct stmt *cs = stmt_create(kind,localdecl,0,0,0,body,0,0);
    return cs;
}
...
{
    struct stmt *ifs = stmt_create(STMT_IF_ELSE,0,0,expr,0,body,0,0);
    return ifs;
}



struct stmt * if_else_create(struct expr *expr, struct stmt *body, struct stmt *else_body)
{
    struct stmt *ifs = stmt_create(STMT_IF_ELSE,0,0,expr,0,body,else_body,0);
    return ifs;
}
...
{
  struct stmt *ws = stmt_create(STMT_WHILE,0,0,expr,0,body,0,0);
  return ws;
}
...
{
    struct expr *e2 = expression_create(kind,left,right,0,0);
    return e2;
}
...
struct expr * expr_create_uni(char *n, struct expr *indexexpr)
{
    struct expr *name = expr_create_name(n);
...
{
    struct expr *arg = expr_create(EXPR_ARG,expr,next);
    return arg;
}
